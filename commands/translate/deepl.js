...
const execute = async function (context, query) {
	...
	if (context.params.from) {
		const code = LanguageParser.getCode(context.params.from);
		if (!code) {
			return {
				success: false,
	...
	if (context.params.to) {
		if (context.params.to === "random") {
			...
			targetLanguageCode = LanguageParser.getCode(context.params.to);
		}
	}
	else {
	...
	if (!targetLanguageCode) {
		...
		const languageName = sb.Utils.capitalize(LanguageParser.getName(targetLanguageCode));
		return {
			success: false,
	...
	if (context.params.formality) {
		...
		if (!allowedFormalities.includes(context.params.formality)) {
			...
			const languageNames = formalitySupportedLanguages
				.map(i => sb.Utils.capitalize(LanguageParser.getName(i)))
				.sort();
	...
	const fromLanguageName = sb.Utils.capitalize(LanguageParser.getName(data.detected_source_language));
	const toLanguageName = sb.Utils.capitalize(LanguageParser.getName(searchParams.target_lang));
