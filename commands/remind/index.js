module.exports = {
	...
	Dynamic_Description: (async (prefix) => {
		return [
			...
			`${prefix}remind (person) hello :)`,
			`${prefix}notify (person) hello :)`,
			...
			`${prefix}remindprivate (person) hello :)`,
			`${prefix}remind private:true (person) hello :)`,
			"Privately reminds target person (via whispers/PMs) when they next type in chat with Supibot.",
			"",

			`${prefix}remind (person) hello :) in (time)`,
			`${prefix}remind supinic hello, how's it going? in 5 minutes`,
			`${prefix}remind supinic in 1 year, 3 months and 5 days hi to the future! `,
			...
			`${prefix}remindme test (time)`,
			`${prefix}remind me check pasta in 5 minutes`,
			"Creates a timed reminder for yourself, much like a regular timed reminder.",
			"",

			`${prefix}remind (person) test on:8pm`,
			`${prefix}remindme fireworks at:"July 4th 2021, 8pm"`,
			`${prefix}remind (person) test on:"tomorrow 10am"`,
